...
import { User, MapPin, ShoppingBag, LogOut, Save, Loader2, CheckCircle2, Package, ChevronDown, ChevronUp } from 'lucide-react';
...
// ── Sección Perfil ────────────────────────────────────────────────────────────
const SeccionPerfil = () => {
  const usuario = useUserAuthStore((s) => s.usuario);
  return (
...
const SeccionEntrega = () => {
  ...
  const { register, handleSubmit, reset, formState: { errors, isSubmitting } } = useForm<DatosEntrega>();

  useEffect(() => {
    const guardados = localStorage.getItem('gifty-datos-entrega');
    if (guardados) reset(JSON.parse(guardados));
  }, [reset]);

  const onSubmit = async (datos: DatosEntrega) => {
    localStorage.setItem('gifty-datos-entrega', JSON.stringify(datos));
    setGuardado(true);
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="sm:col-span-2">
          ...
          <input {...register('direccion', { required: 'Requerida' })} placeholder="Ej: Av. Corrientes 1234" className={inputCls} />
        ...
        <div>
          ...
          <input {...register('ciudad', { required: 'Requerida' })} placeholder="Ej: Buenos Aires" className={inputCls} />
        ...
        <div>
          ...
          <input {...register('provincia', { required: 'Requerida' })} placeholder="Ej: CABA" className={inputCls} />
        ...
        <div>
          ...
          <input {...register('telefono', { required: 'Requerido' })} placeholder="Ej: 11 1234-5678" className={inputCls} />
        ...
        <div>
          ...
          <input {...register('empresa')} placeholder="Ej: Distribuidora Sol" className={inputCls} />
        </div>
      </div>
      ...
      <button
        ...
        style={{ background: guardado ? '#10b981' : '#e91e8c' }}
      >
